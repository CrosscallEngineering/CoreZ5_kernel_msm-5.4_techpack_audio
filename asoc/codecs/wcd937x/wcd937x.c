...
/*zsq add for handset monitor status start*/
#ifdef CONFIG_HANDSET_MONITOR_KEY
#include <linux/input.h>
int handset_mode_flag=0;
#endif
...
#endif
/*zsq add for handset monitor status end*/
...
/*zsq add for handset monitor status start*/
#ifdef CONFIG_HANDSET_MONITOR_KEY
void handset_monitor_select(struct wcd937x_priv *wcd937x, int handset_monitor_key)
{
	if (wcd937x->input) {
		input_report_key(wcd937x->input, handset_monitor_key,1);
		input_sync(wcd937x->input) ;
		msleep(10);
		input_report_key(wcd937x->input, handset_monitor_key,0);
		input_sync(wcd937x->input) ;
		pr_info("%s handset_monitor_key=%d \n", __func__, handset_monitor_key);
        }
}
#endif
/*zsq add for handset monitor status end*/
...
#ifdef CONFIG_HANDSET_MONITOR_KEY
			handset_mode_flag = 1;
			handset_monitor_select(wcd937x, HANDSET_MONITOR_ON_KEY);
			dev_err(component->dev, "%s zsq add handset on\n", __func__); 			
#endif
		break;
	case SND_SOC_DAPM_PRE_PMD:
		if (wcd937x->ear_rx_path & EAR_RX_PATH_AUX)
...
/*zsq add for handset monitor status start*/
#ifdef CONFIG_HANDSET_MONITOR_KEY
		if(handset_mode_flag == 1)
		{
			handset_monitor_select(wcd937x, HANDSET_MONITOR_OFF_KEY);
			dev_err(component->dev, "%s zsq add handset off\n", __func__);
		}
		handset_mode_flag = 0;	
#endif
		break;
...
/*zsq add for handset monitor status start*/
#ifdef CONFIG_HANDSET_MONITOR_KEY	
		wcd937x->input = input_allocate_device();
		if (wcd937x->input == NULL) {
			dev_err(component->dev,"%s: Failed to allocate input\n", __func__);
			ret = -ENOMEM;
			return ret;
		}
		input_set_capability(wcd937x->input , EV_KEY, HANDSET_MONITOR_ON_KEY);
		input_set_capability(wcd937x->input , EV_KEY, HANDSET_MONITOR_OFF_KEY);
		wcd937x->input->name= "HandsetStatus";
		ret = input_register_device(wcd937x->input);
		if (ret) {
			dev_err(component->dev,"%s: Failed to register input\n", __func__);
			input_free_device(wcd937x->input);
			return ret;
		}
#endif	
/*zsq add for handset monitor status end*/	
